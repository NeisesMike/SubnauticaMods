...
namespace SelfRepairModule
{
    internal static class Configuration
    {
        ...
        internal static void RegisterOptions()
        {
            ...
            ConfigRegistrar.RegisterForCyclops<float>(amountOptionName, new ConfigDescription(amountOptionDescription, new AcceptableValueRange<float>(0, 10)), 5f*35f/150f, null, MainPatcher.Instance.Config);
            ...
            ConfigRegistrar.RegisterForPrawn<float>(costOptionName, new ConfigDescription(costOptionDescription, new AcceptableValueRange<float>(0, 5)), 1, null, MainPatcher.Instance.Config);
            ConfigRegistrar.RegisterForCyclops<float>(costOptionName, new ConfigDescription(costOptionDescription, new AcceptableValueRange<float>(0, 5)), 1, null, MainPatcher.Instance.Config);
            ConfigRegistrar.RegisterForAllModVehicles<float>(costOptionName, new ConfigDescription(costOptionDescription, new AcceptableValueRange<float>(0, 5)), 1, null, MainPatcher.Instance.Config);
        }
    }
}
